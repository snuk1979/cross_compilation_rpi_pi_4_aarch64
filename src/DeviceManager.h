...
class IDeviceManager {
   public:
     ...
     * @param format A string representing the desired buffer format in
     * read/writeStream()
     * @parblock
     *
     ...
     *
     * The type character is followed by the number of bits per number (complex
     * is 2x this size per sample)
     *
    ...
    /**
     * @breif Waits for a thread loop stop signal
     */
    virtual void WaitShutdownSignal() = 0;
    /**
    ...
    virtual SoapySDR::Kwargs GetHardwareInfo(
        const int deviceNumber = 0) const = 0;
    /**
    ...
    auto CallThreadSafe(Lock& lock, IDMO obj, Fn fn, Arg&&... args) const
        -> decltype((obj->*fn)(std::forward<Arg>(args)...)) {
